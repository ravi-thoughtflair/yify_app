...
export class YifyMoviesProvider {
  ...
  queryjson: any = {
    rating_list: [
      {
        elem: '.container .row .table-responsive .other-subs tbody .rating-cell ',
        rating: 'text',
      },
    ],
    lang_list: [
      {
        elem: '.container .row .table-responsive .other-subs tbody .flag-cell .sub-lang ',
        lang: 'text',
      },
    ],
    sub_links: [
      {
        elem: '.container .row .table-responsive .other-subs tbody  .download-cell a',
        download_link: 'href',
      },
    ],
    sub_names: [
      {
        elem: '.container .row .table-responsive .other-subs tbody td',
        subname: 'text',
      },
    ],
  };
  subscene_queryjson: any = {
    down_links: [{ elem: '.a1 > a:first-of-type', link: 'href' }],
    langs_names: [
      { elem: '.a1 > a:first-of-type span', link: 'href', lang_name: 'text' },
    ],
  };
  subscene_download_json: any = {
    downlink: [{ elem: '.download > a:first-of-type', link: 'href' }],
  };
  Tmdburl: any =
  ...
  loadYifyMovies(
    ...
    return this.http
      .get(
        `${this.yifysites[sitecount]}/list_movies.jsonp?limit=${limit}&page=${page}`
      )
  ...
  loadYifyTopRatedMovies(
    ...
    return this.http
      .get(
        `${this.yifysites[sitecount]}/list_movies.jsonp?sort_by=rating&limit=${limit}&page=${page}`
      )
      .map((res) => <Response>res.json())
      .catch((error: any) => {
        return Observable.throw(new Error(error.status));
  ...
  loadYifyMovieDetails(movie_id: number): Observable<Response> {
    return this.http
      .get(
        `${this.Url}/movie_details.json?movie_id=${movie_id}&with_images=true&with_cast=true`
      )
      .map((res) => <Response>res.json())
      .catch((error: any) => {
        return Observable.throw(new Error(error.status));
  ...
  searchYifyMovies(query_term: string): Observable<Response> {
    return this.http
      .get(
        `${this.Url}/list_movies.jsonp?sort_by=rating&limit=30&query_term=${query_term}`
      )
      .map((res) => <Response>res.json())
      .catch((error: any) => {
        return Observable.throw(new Error(error.status));
  ...
  advancedSearchYifyMovies(
    ...
    return this.http
      .get(
        `${this.Url}/list_movies.jsonp?limit=${limit}&page=${page}&${criteria}`
      )
      .map((res) => <Response>res.json())
      .catch((error: any) => {
        return Observable.throw(new Error(error.status));
  ...
  getYifySUbTitles(imdb_code: string): Observable<any> {
    ...
    return this.http
      .post(`${this.ServerIp}`, body, options)
  ...
  searchTMDbQueryData(
    ...
    return this.http
      .get(`${this.Tmdburl}&query=${query_term}&page=${page}`)
